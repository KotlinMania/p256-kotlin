# Claude Code Project Instructions — p256-kotlin

## Project Overview

This is **p256-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`p256`](https://crates.io/crates/p256).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/p256/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/p256>
